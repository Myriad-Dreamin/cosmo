...
Pure CMake discovery uses `COSMO_LLVM_PATH` as the LLVM/Clang install prefix. The value may come from the environment, a `.env` file applied by the driver, or from `-DCOSMO_LLVM_PATH=<prefix>`. When no path is configured, the `packages/cosmo-clang-sys` CMake package reads `packages/cosmo-clang-sys/config/llvm-manifest.json`, selects the matching LLVM `21.1.8` entry for the host platform and architecture, downloads the `llvm-dist` component archives into `target/cosmo/llvm`, verifies their SHA-256 digests, extracts them into one install prefix, and uses that install's `include` and `lib` directories. The default manifest combines `llvm-core`, `clang-sdk`, `clang-tooling`, and `clang-repl`. If the configured install also contains LLVM/Clang CMake package files, `cosmo-clang-sys` uses those package targets; otherwise it imports the static LLVM/Clang libraries from the install layout directly.
