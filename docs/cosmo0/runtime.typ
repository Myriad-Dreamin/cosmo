...
Pure CMake discovery uses `COSMO_LLVM_PATH` as the LLVM/Clang install prefix. The value may come from the environment, a `.env` file applied by the driver, or from `-DCOSMO_LLVM_PATH=<prefix>`. When no path is configured, the `packages/cosmo-clang-sys` CMake package reads `packages/cosmo-clang-sys/config/llvm-manifest.json`, selects the matching LLVM `21.1.8` artifact for the host platform and architecture, downloads it from `clice-llvm` releases into `target/cosmo/llvm`, verifies the SHA-256 digest, extracts it, and uses that install's `include` and `lib` directories. If the configured install also contains LLVM/Clang CMake package files, `cosmo-clang-sys` uses those package targets; otherwise it imports the static LLVM/Clang libraries from the install layout directly.
