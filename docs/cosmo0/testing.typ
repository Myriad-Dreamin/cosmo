...
Profile selection is currently a development and test-harness feature. `Cosmo0.checkWithProfile` and package `checkerProfile` metadata can select `cosmo0.subset`, and `mltt.core` routes directly to `MlttTypeChecker` for source assertion fixtures. Ordinary Cosmo source still requires the default source checker until a surface elaborator can produce MLTT core artifacts. The MLTT core checker itself is tested through `packages/cosmoc/src/types/mltt/` fixture programs and the Scala mirror at `packages/cosmo0/src/main/scala/cosmo0/tyck/mltt/TypeChecker.scala`.
