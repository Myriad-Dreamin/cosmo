...
The accepted build artifact is a Linux static library named `libcosmoClang.a`, produced by the `packages/cosmo-clang-sys` CMake target `cosmoClang`. The target exports the C ABI declared in `packages/cosmo-clang-sys/include/cosmo_clang_sys.h`. The primary query accepts a canonical namespace, a qualified suffix, and a bounded header set, then uses Clang to parse a probe translation unit that includes those headers and references the requested symbol.

Pure CMake discovery uses `COSMO_LLVM_PATH` as the LLVM/Clang install prefix. The value may come from the environment, a `.env` file applied by the driver, or from `-DCOSMO_LLVM_PATH=<prefix>`. When no path is configured, the `packages/cosmo-clang-sys` CMake package reads `packages/cosmo-clang-sys/config/llvm-manifest.json`, selects the matching LLVM `21.1.8` entry for the host platform and architecture, downloads the manifest archives into `target/cosmo/llvm`, verifies their SHA-256 digests, extracts them into one install prefix, and uses that install's `include` and `lib` directories. The default manifest uses `llvm-dist` split archives combining the Clang libraries and tooling needed by `cosmo-clang-sys`, with optional `clang-repl` archives allowed only when a published distribution already includes them. Compile-time evaluation does not require `clang-repl` because compiler-local eval modules use provider-entry compilation against cached PCH/precompiled contexts. The manifest retains legacy single-archive `clice-llvm` fallbacks for release targets that are not published through `llvm-dist` yet. If the configured install also contains LLVM/Clang CMake package files, `cosmo-clang-sys` uses those package targets; otherwise it imports the static LLVM/Clang libraries from the install layout directly.

`COSMO_LLVM_PATH` remains the override for locally managed toolchains. `COSMO_LLVM_OFFLINE=ON` disables network access and requires either a configured path or an already-populated `target/cosmo/llvm` cache. Packages that do not request `cosmo-clang-sys` do not configure this CMake package and do not require LLVM.

On Linux, downloaded LLVM/Clang static libraries may require a newer or matching GNU toolchain. If the final native link reports missing `__isoc23_*`, `arc4random`, or newer `GLIBC_2.xx` symbols, configure `COSMO_GCC_TOOLCHAIN` or `GCC_TOOLCHAIN` to the matching GCC root, such as `/usr/local/gcc-14.3.0`. The `cosmoClang` CMake target passes the value to Clang as `--gcc-toolchain` and links `libgcc` and `libstdc++` statically by default. A locally managed LLVM build should also set `COSMO_LLVM_PATH` and `CC`/`CXX` to the matching Clang install before the CMake build directory is configured. Use `COSMO_CMAKE_C_COMPILER` and `COSMO_CMAKE_CXX_COMPILER` when the package driver should pass compiler paths as explicit CMake cache entries.

The real `cosmoc` executable is the `packages/cosmoc` package built by `node cmd/cosmo/main.js -p packages/cosmoc build -o target/cosmoc`. Its package metadata requests `cosmo-clang-sys` as a native support library, so the package driver generates a small CMake project for the final host link and links that executable target against `cosmoClang`. Generated backend requirement records for C++ namespace imports include both header requirements and a `cpp-namespace-import:*` requirement so the driver can route validation through this library before host linking.
