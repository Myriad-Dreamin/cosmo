...
class ParserFixtureTests extends munit.FunSuite:
  test(
    "shared parser fixture directives are well formed and point at existing fixtures",
  ):
    val fixtures = ParserFixtureManifest.load()

    assert(
      fixtures.nonEmpty,
      "shared parser fixture directives must list at least one fixture",
    )
    assertEquals(fixtures.map(_.id).distinct.length, fixtures.length)
    assert(
      fixtures.exists(
        _.expectedStatus == ParserFixtureManifest.ExpectedStatus.Ok,
      ),
      "shared parser fixture directives must include an accepted fixture",
    )
    assert(
      fixtures.exists(
        _.expectedStatus == ParserFixtureManifest.ExpectedStatus.Error,
      ),
    ...
    fixtures.foreach: fixture =>
      assert(
        ...
        s"missing parser fixture ${fixture.path}",
      )
      ...
      .foreach: fixture =>
        ...
        fixture.expectedStatus match
          case ParserFixtureManifest.ExpectedStatus.Ok =>
            ...
            assert(
              result.isSuccess,
              s"${fixture.id} should parse but got ${result.diagnostics
          ...
          case ParserFixtureManifest.ExpectedStatus.Error =>
            ...
            fixture.diagnosticCode.foreach: code =>
              assert(
                ...
                s"${fixture.id} should report $code but got ${result.diagnostics
                    .map(_.code)}",
              )

  test(
    "native parser AST fixture directives are well formed and point at existing fixtures",
  ):
    val fixtures = ParserFixtureManifest.loadAst()

    assert(
      fixtures.nonEmpty,
      "native parser AST fixture directives must list at least one fixture",
    )
    assertEquals(fixtures.map(_.id).distinct.length, fixtures.length)
    assert(
      fixtures.exists(
    ...
    fixtures.foreach: fixture =>
      assert(
        ParserFixtureManifest.exists(fixture.path),
        s"missing parser AST fixture ${fixture.path}",
      )
