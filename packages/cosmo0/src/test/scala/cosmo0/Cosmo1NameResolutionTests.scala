...
class Cosmo1NameResolutionTests extends munit.FunSuite:
  ...
  private val resolutionTestPath =
    "packages/cosmoc/src/names/resolution_test.cos"
  ...
  test("cosmo1 name resolution source compiles with parser subset fixtures"):
    ...
    assert(
      compiled.isSuccess,
      s"cosmo1 name resolution compile failed with diagnostics: ${compiled.diagnostics
    ...
    assert(
      output.contains(
        "inline bool name_resolution_test_resolves_parser_subset_fixture()",
      ),
    )
    assert(
      output.contains(
        "inline bool name_resolution_test_reports_duplicate_definition()",
      ),
    )
    assert(
      output.contains(
        "inline bool name_resolution_test_allows_repeated_same_import()",
      ),
    )
    assert(
      output.contains(
        "inline bool name_resolution_test_reports_unresolved_name()",
      ),
    )
