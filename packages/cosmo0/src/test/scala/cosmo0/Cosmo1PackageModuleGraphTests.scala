...
class Cosmo1PackageModuleGraphTests extends munit.FunSuite:
  ...
  test("cosmo1 package graph fixtures load explicit sources and order dependencies"):
    ...
    assert(
      loaded.isSuccess,
      s"cosmo1 package graph fixture load failed: ${loaded.diagnostics.map(d => d.code -> d.message)}",
    )
    assertEquals(loaded.value.get.metadata.stageProfile, Some(StageCapabilityRegistry.Cosmo1Stage1))
    assertEquals(loaded.value.get.metadata.sourceFiles, Some(List("main.cos", "util.cos")))
    assertEquals(loaded.value.get.modules.map(_.modulePath), List(List("main"), List("util")))
    ...
    assert(
      checked.isSuccess,
      s"cosmo1 package graph fixture check failed: ${checked.diagnostics.map(d => d.code -> d.message)}",
    )
    assertEquals(checked.value.get.moduleOrder, List("util", "main"))

  test("cosmo1 package graph fixtures diagnose missing imports and cycles before checking"):
    val missingImport = Cosmo0().checkPackage("fixtures/cosmo0/cosmo1/package-graph/missing-import")
    ...
    assert(
      ...
      s"missing import diagnostic not found: ${missingImport.diagnostics.map(d => d.code -> d.message)}",
    )

    val cycle = Cosmo0().checkPackage("fixtures/cosmo0/cosmo1/package-graph/cycle")
  ...
  test("single-source parser validation can bypass package metadata"):
    ...
    val checked = Cosmo0().check(SourceFile(ParserFixtureManifest.parserSourcePath, source))

    assertEquals(checked.phase, Phase.Check)
    assert(
      checked.isSuccess,
      s"single-source parser validation failed: ${checked.diagnostics.map(d => d.code -> d.message)}",
    )
    assert(checked.value.get.typed.declarations.exists(_.name == "SyntaxParserResult"))
    assert(checked.value.get.typed.declarations.exists(_.name == "parse_source_ast"))
