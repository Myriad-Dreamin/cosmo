...
class Core0CommandTests extends munit.FunSuite:
  ...
  test(
    "core0.command source API lowers through trusted externs, not descriptors",
  ):
    ...
    val lowered =
      Cosmo0().lower(SourceFile("core0_command_run_smoke.cos", source))

    assertEquals(lowered.phase, Phase.Compile)
    assert(
      lowered.isSuccess,
      s"core0.command lowering failed with diagnostics: ${lowered.diagnostics
    ...
    assert(
      rendered.contains(
        "fn @command_run command_run(%command command: &Command) -> Result[CommandResult, CommandError] extern cosmo0.extern.v0 \"::cosmo0_runtime::command_run\"",
      ),
    )
    ...
    assert(
      compiled.isSuccess,
      s"core0.command C++ emission failed with diagnostics: ${compiled.diagnostics
          .map(d => d.code -> d.message)}",
    )
    assert(
      compiled.value.get.backendRequirements.contains(
        BackendRequirement.runtimeSymbol("cosmo0_runtime::command_run"),
      ),
    )
    assert(
      compiled.value.get.backendRequirements
        .contains(BackendRequirement.include("<cstdlib>")),
    )
    assert(
      compiled.value.get.backendRequirements
        .contains(BackendRequirement.include("<string>")),
    )
    assert(
      compiled.value.get.backendRequirements
        .contains(BackendRequirement.include("<vector>")),
    )
    assert(
      !compiled.value.get.backendRequirements
        .contains(BackendRequirement.include("<sys/wait.h>")),
    )
    assert(
      compiled.value.get.source.contains(
        "#if defined(_WIN32)\n#include <process.h>\n#else\n#include <sys/wait.h>\n#endif",
      ),
    )
    assert(
      compiled.value.get.source.contains("return _popen(line.c_str(), \"r\");"),
    )
    assert(compiled.value.get.source.contains("return _pclose(pipe);"))

  test(
    "core0.command source tests and link component compile as later-stage source",
  ):
    val source = combineSources(
      List(
        pathFsPath,
    ...
    val lowered =
      Cosmo0().lower(SourceFile("core0_command_component_smoke.cos", source))

    assertEquals(lowered.phase, Phase.Compile)
    assert(
      lowered.isSuccess,
      s"core0.command component lowering failed with diagnostics: ${lowered.diagnostics
          .map(d => d.code -> d.message)}",
    )

  test(
    "core0.command capability is later-stage std metadata, not a descriptor family",
  ):
    ...
    val availability = StageCapabilityRegistry.defaultAvailability.copy(
      stdCapabilities =
        StageCapabilityRegistry.defaultAvailability.stdCapabilities - StageCapabilityRegistry.Core0Command,
    )
    ...
    assert(
      diagnostics.exists(
        _.message.contains("requires std capability core0.command"),
      ),
    )
    assert(
      StageCapabilityRegistry.knownStdCapabilities.contains(
        StageCapabilityRegistry.Core0Command,
      ),
    )
    assert(
