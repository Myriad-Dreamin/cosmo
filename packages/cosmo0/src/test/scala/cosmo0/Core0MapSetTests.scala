...
class Core0MapSetTests extends munit.FunSuite:
  ...
  private val moduleGraphTestPath =
    "packages/cosmoc/src/package/module_graph_test.cos"

  test(
    "core0.map-set source API compiles through deterministic ordered backend containers",
  ):
    ...
    assert(
      compiled.isSuccess,
      s"core0.map-set source test compile failed with diagnostics: ${compiled.diagnostics
    ...
    assert(
      output.contains("inline bool map_set_test_iteration_is_key_sorted()"),
    )
    assert(
      output.contains("inline bool map_set_test_set_iteration_is_sorted()"),
    )
  ...
  test("Map and Set iteration lower to deterministic descriptor iteration"):
    ...
    assert(
      lowered.isSuccess,
      s"core0.map-set lowering failed with diagnostics: ${lowered.diagnostics
          .map(d => d.code -> d.message)}",
    )
  ...
  test("Map and Set reject unsupported key types"):
    ...
    val keyDiagnostics =
      result.diagnostics.filter(_.code == "cosmo0.type.unsupported-map-key")
  ...
  test(
    "Map and Set accept supported key types including aliases and typed IDs",
  ):
    val result = Cosmo0().check(
      """class Definition {}
        |type Symbol = String
    ...
    assert(
      result.isSuccess,
      s"supported map/set key types should check: ${result.diagnostics
          .map(d => d.code -> d.message)}",
    )

  test(
    "cosmo1 module graph and name scope compile with deterministic maps and sets",
  ):
    ...
    assert(
      compiled.isSuccess,
      s"cosmo1 package/name map-set compile failed with diagnostics: ${compiled.diagnostics
          .map(d => d.code -> d.message)}",
    )
    val output = compiled.value.get.output
    ...
    assert(output.contains("std::set<std::string>"))
    assert(
      output.contains("inline bool module_graph_test_uses_deterministic_maps()"),
    )
    assert(
      output.contains("inline bool module_graph_test_loads_explicit_sources()"),
    )
    assert(
      output.contains(
        "inline bool module_graph_test_orders_dependencies_first()",
      ),
    )

  test(
    "core0.map-set capability is later-stage std metadata, not a Stage 1 requirement",
  ):
    ...
    val availability = StageCapabilityRegistry.defaultAvailability.copy(
      stdCapabilities =
        StageCapabilityRegistry.defaultAvailability.stdCapabilities - StageCapabilityRegistry.Core0MapSet,
    )
    ...
    assert(
      diagnostics.exists(
        _.message.contains("requires std capability core0.map-set"),
      ),
    )
    assert(
      StageCapabilityRegistry.knownStdCapabilities.contains(
        StageCapabilityRegistry.Core0MapSet,
      ),
    )
    assert(
