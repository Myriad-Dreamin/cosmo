...
class Cosmo1DeclarationResolutionTests extends munit.FunSuite:
  ...
  private val declarationResolutionTestPath = "packages/cosmoc/src/types/declaration_resolution_test.cos"
  ...
  test("cosmo1 declaration resolution source compiles"):
    ...
    val compiled = Cosmo0().compile(SourceFile(declarationResolutionTestPath, source))

    assertEquals(compiled.phase, Phase.Compile)
    assert(
      compiled.isSuccess,
      s"cosmo1 declaration resolution compile failed with diagnostics: ${compiled.diagnostics.map(d => d.code -> d.message)}",
    )
    ...
    assert(output.contains("inline DeclarationResolution resolve_module_declarations("))
    assert(output.contains("inline bool declaration_resolution_test_records_parser_state_and_helpers()"))
    assert(output.contains("inline bool declaration_resolution_test_reports_unknown_type()"))
    assert(output.contains("inline bool declaration_resolution_test_reports_alias_cycle()"))
    assert(output.contains("inline bool declaration_resolution_test_rejects_invalid_receiver()"))
