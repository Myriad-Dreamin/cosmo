...
- [ ] 1.2 Add `docs/cosmo/compile-time-evaluation.typ` covering macro function input/output records, macro output purity, full C++ compile-time execution through `cosmo-cte-sys`, and target runtime separation.
...
- [ ] 6.6 Route C++ compile-time execution through `cosmo-cte-sys` with provider-entry compilation, PCH/precompiled context reuse, imported C++ types, provider code execution, structured diagnostics, and Clang-owned layout facts.
