...
- **AND** the profile lists the extern/backend requirements needed by Stage 1 source loading and deterministic text output
...
- **THEN** validation fails with a missing-capability diagnostic naming the missing capability
...
- **THEN** validation fails with a missing-capability diagnostic naming the missing primitive descriptor
...
- **THEN** validation fails with a missing-capability diagnostic naming the missing backend requirement
