...
The support-library pipeline SHALL define lowercase kebab-case support-library ids, C symbol prefixes, artifact names, and a shared staged output layout. Static support-library artifacts SHALL be staged under `target/cosmo/support-libraries/<profile>/<support-library-id>/`.
